...
RSpec.describe RobotApplication::Command::Move::Base do
  ...
  let(:table) { double("table", contains_coordinates?: true) }
  let(:x) { 1 }
  let(:y) { 2 }
  ...
  describe "#execute" do
    it "updates the robot's position if the table contains the next position" do
      expect(table).to receive(:contains_coordinates?).with(x: x, y: y).and_return(true)
    ...
    it "does not update the robot's position if the table does not contain the next position" do
      expect(table).to receive(:contains_coordinates?).with(x: x, y: y).and_return(false)
